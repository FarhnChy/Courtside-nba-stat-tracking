...
async function getHtml(url, ttl = 1_800_000) {
  ...
  const response = await fetch(url, { headers: { 'user-agent': 'Courtside/0.2 portfolio project' } });
  if (!response.ok) throw new Error(`Contract source returned ${response.status}`);
...
async function getJson(url, ttl = 300_000) {
  ...
  const response=await fetch(url,{headers:{accept:'application/json','user-agent':'Courtside/0.2 portfolio project'}});if(!response.ok)throw new Error(`Roster source returned ${response.status}`);
...
async function freeAgents() {
  ...
  const players = (page.players || []).map(player => {
    ...
    type: String(player.type || '').toUpperCase(), availability: currentTeam||player.availability==='S'?'Signed':'Available', oldTeam: player.oldTeamAbbr || null, newTeam: currentTeam||player.newTeamAbbr||null, reconciled:currentTeam?'ESPN current roster':null,
    ...
    profile: player.playerLink || null, article: player.articleLink || null
  })});
