...
async function freeAgents() {
  ...
  let players = (page.players || []).map(player => {
    ...
    const verifiedMove=trackerSigned||Boolean(reportedTeam)||Boolean(rosterChanged&&(player.articleLink||player.articleUrl));
    return ({
    ...
    type: String(player.type || '').toUpperCase(), availability: verifiedMove?'Signed':'Available', oldTeam: player.oldTeamAbbr || null, newTeam: verifiedMove?(reportedTeam||normalizedCurrentTeam||normalizedTrackerTeam||null):null, reconciled:verifiedMove?(trackerSigned?'NBA free-agent tracker':'ESPN roster + NBA report'):null,
