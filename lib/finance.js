...
function reportedTeamFromLabel(label = '') {
  const matches = [...String(label).toUpperCase().matchAll(/\b(?:WITH|TO)\s+([A-Z]{2,4})\b/g)];
  for (const match of matches.reverse()) {
    const code = espnToNba[match[1]] || match[1];
    if (teamSlugs[code]) return code;
  }
  const teamToken = [...String(label).toUpperCase().matchAll(/\b[A-Z]{2,4}\b/g)]
    .map(match => espnToNba[match[0]] || match[0])
    .find(code => teamSlugs[code]);
...
async function getHtml(url, ttl = 1_800_000) {
  ...
  let lastError;
  for(let attempt=0;attempt<2;attempt++)try{const response=await fetch(url,{headers:{'user-agent':'RimRelay/0.2 portfolio project'},signal:AbortSignal.timeout(15_000)});if(!response.ok)throw new Error(`Contract source returned ${response.status}`);const data=await response.text();cache.set(url,{time:Date.now(),data});return data}catch(error){lastError=error}
...
async function freeAgents() {
  ...
  const retiredPlayerIds = new Set(offseasonStatusUpdates.filter(update => update.type === 'Retired').map(update => String(update.player.id)));
  const retiredPlayerNames = new Set(offseasonStatusUpdates.filter(update => update.type === 'Retired').map(update => update.player.name.toLowerCase()));
  const contractReports = new Map(offseasonContracts
    .filter(entry => /signed|re-signed|sign-and-trade/i.test(entry.type))
    .map(entry => [entry.player.toLowerCase(), entry]));
  let players = (page.players || []).map(player => {
    ...
    const contractReport=contractReports.get(String(player.playerDisplayName||'').toLowerCase());
    const reportedTeam=reportedTeamFromLabel(player.articleLabel);
    const normalizedCurrentTeam=espnToNba[currentTeam]||currentTeam;
    const normalizedTrackerTeam=espnToNba[player.newTeamAbbr]||player.newTeamAbbr;
    ...
    const verifiedMove=trackerSigned||Boolean(reportedTeam)||Boolean(contractReport)||Boolean(rosterChanged&&(player.articleLink||player.articleUrl));
    return ({
    ...
    type: String(player.type || '').toUpperCase(), availability: verifiedMove?'Signed':'Available', oldTeam: player.oldTeamAbbr || null, newTeam: verifiedMove?(reportedTeam||contractReport?.team||normalizedCurrentTeam||normalizedTrackerTeam||null):null, reconciled:verifiedMove?(trackerSigned?'NBA free-agent tracker':contractReport?'ESPN/Shams reported contract':'ESPN roster + NBA report'):null,
  ...
  })}).filter(player => !retiredPlayerIds.has(player.id) && !retiredPlayerNames.has(player.name.toLowerCase()));
