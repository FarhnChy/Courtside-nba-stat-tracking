...
async function freeAgents() {
  ...
  const players = (page.players || []).map(player => ({
    ...
    type: String(player.type || '').toUpperCase(), availability: player.availability === 'S' ? 'Signed' : 'Available', oldTeam: player.oldTeamAbbr || null, newTeam: player.newTeamAbbr || null,
