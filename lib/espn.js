...
async function getJson(url, ttl = 15_000) {
  ...
  for (let attempt=0;attempt<2;attempt++) try {
    const response = await fetch(url, { headers: { accept: 'application/json', 'user-agent': 'Courtside/0.2' }, signal:AbortSignal.timeout(12_000) });
...
module.exports = { scoreboard, scheduleWindow, summary, standings, teamsList, roster, injuries, transactions, playerOverview, normalizeEvent, normalizeSummary, normalizeStandings, normalizeTeams, normalizeRoster, normalizeInjuries, normalizeTransactions, normalizePlayerOverview, normalizePlayerHistory };
