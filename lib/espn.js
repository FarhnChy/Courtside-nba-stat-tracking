...
async function roster(teamId) {
  ...
  try {
    ...
    normalized.coaches = (set?.rowSet||[]).map(row=>({ id:String(row[index.COACH_ID]), name:row[index.COACH_NAME], role:row[index.COACH_TYPE]||'Coach', assistant:Number(row[index.IS_ASSISTANT])>1, headshot:`https://cdn.nba.com/headshots/nba/latest/1040x760/${row[index.COACH_ID]}.png` }));
  } catch (_) {}
  return normalized;
}
