...
for (const file of ['public/index.html', 'public/styles.css', 'public/app.js', 'public/rimrelay.svg']) {
...
assert.equal(offseasonStatusUpdates.some(move => move.player.name === 'Russell Westbrook' && move.type === 'Retired'), true, 'manual status updates should include retirements');
const offseasonReportedTransactions = require('../data/offseasonReportedTransactions');
assert.equal(offseasonReportedTransactions.some(move => move.player.name === 'Dennis Schroder' && move.team === 'CHA'), true, 'late reports should include team-scoped transaction updates');
...
assert.deepEqual(offseasonFreeAgents.map(player => player.name), []);
...
assert.equal(serverSource.includes('/api/shams-updates'), true, 'server should expose the ESPN/Shams roster news feed');
...
assert.equal(appSource.includes('move-verification'), true, 'transaction cards should render proof status');
assert.equal(appSource.includes('manualVerifiedAt'), true, 'transaction feed should expose manual verification context');
assert.equal(serverSource.includes('verificationFor'), true, 'transaction feed should proof-check live reports against actual sources');
...
for (const feature of ['loadShamsUpdates','shamsUpdateList','/api/shams-updates']) assert.equal(appSource.includes(feature) || html.includes(feature), true, `Shams/ESPN update feed should include ${feature}`);
...
assert.equal(html.includes('brand-wordmark'), true, 'topbar should display the RimRelay wordmark');
assert.equal(html.includes('<title>RimRelay</title>'), true);
